...
public class View extends GameView {
  ...
  protected void _paint(Graphics g) {
    ...
    Iterator<Plateforme> iterp = m_model.plateformes();
    while(iterp.hasNext()) {
    	Plateforme p = iterp.next();
    	p.paint(g);
    }
    
    m_model.m_hero.paint(g);
    
    Iterator<Ghost> iterg = m_model.ghosts();
    while(iterg.hasNext()) {
    	Ghost ghost = iterg.next();
    	ghost.paint(g);
    }
    
    Iterator<Missile> iterm = m_model.missiles();
    while(iterm.hasNext()) {
